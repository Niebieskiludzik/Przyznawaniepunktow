document.addEventListener("DOMContentLoaded", async () => {
  ...
  const hRankingList = document.getElementById("hRankingList");
  ...
  // 📅 domyślny miesiąc
  const now = new Date();
  yearPicker.value = now.toISOString().slice(0, 7);
  ...
  async function loadRanking() {
    ...
    hRankingList.innerHTML = "Ładowanie...";

    const [year] = yearPicker.value.split("-").map(Number);
